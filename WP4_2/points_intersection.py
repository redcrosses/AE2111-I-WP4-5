...
def find_intersections(airfoil_points, y_vals):
    ...
    frontspar1 = np.concatenate(intersection(np.ones(len(airfoil_x)) * y_vals[0], np.linspace(0, 1, len(airfoil_x)), airfoil_x, airfoil_y), dtype=object)
    # print(frontspar1)
    frontspar2 = np.concatenate(intersection(np.ones(len(airfoil_x)) * y_vals[0], np.linspace(-1, 0, len(airfoil_x)), airfoil_x, airfoil_y ), dtype=object)
    # print(frontspar2)
    rearspar1 = np.concatenate(intersection(np.ones(len(airfoil_x)) * y_vals[1], np.linspace(0, 1, len(airfoil_x)), airfoil_x, airfoil_y), dtype=object)
    rearspar1 = [rearspar1[0], rearspar1[2]]
    # print(rearspar1)
    rearspar2 = np.concatenate(intersection(np.ones(len(airfoil_x)) * y_vals[1], np.linspace(-1, 0, len(airfoil_x)), airfoil_x, airfoil_y), dtype=object)
    rearspar2 = [rearspar2[0], rearspar2[2]]
    # print(rearspar2)
